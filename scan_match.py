...
class PauseAndCapture(Node):
    ...
    def transform_pointcloud2(self, cloud_msg: PointCloud2, transform: TransformStamped) -> list[tuple[int, int, int]]:
        ...
        def rotate_point_euler(x_value, y_value, z_value, roll, pitch, yaw) -> tuple[int, int, int]:
            ...
            return tuple(combined_rotation_matrix @ np.array([x_value, y_value, z_value]))
        ...
        transformed_points = []
        for pt_value in pc2.read_points(cloud_msg, field_names=("x", "y", "z"), skip_nans=True):
            # Get values of pt
            x_value, y_value, z_value = pt_value


            # Apply rotation to the point using Euler angles use the rotate point euler function
            rotated_point = rotate_point_euler(x_value, y_value, z_value,
    ...
    def perform_icp(self, previous_points, current_points, max_iterations=20, tolerance=1e-4):
        ...
        def svd_estimation(previous_points, current_points):
            ...
            # Center both
            p_array = current_points - source_centroid
            q_array = previous_points - target_centroid

            # Cross-covariance
            h_array = p_array.T @ q_array
            u_array, _, vt_matrix = np.linalg.svd(h_array)
            r_opt = vt_matrix.T @ u_array.T

            # Fix reflection case
            if np.linalg.det(r_opt) < 0:
                vt_matrix[2, :] *= -1
                r_opt = vt_matrix.T @ u_array
