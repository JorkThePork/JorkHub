...
class PauseAndCapture(Node):
    ...
    def delayed_transform_lookup(self):
        ...
        try:
            ...
            transformed_points = self.transform_pointcloud2(cloud_in_laser, transform) # Transform the point cloud with the transform_pointclod2 function
    ...
    def transform_pointcloud2(self, cloud_msg: PointCloud2, transform: TransformStamped) -> list[tuple[int, int, int]]:
        ...
        def rotate_point_euler(x, y, z, roll, pitch, yaw) -> tuple[int, int, int]:
            ...
            roll_array = np.array([[1, 0, 0], [0, np.cos(roll), -np.sin(roll)], [0, np.sin(roll), np.cos(roll)]])
            pitch_array = np.array([[np.cos(pitch), 0, np.sin(pitch)], [0, 1, 0], [-np.sin(pitch), 0, np.cos(pitch)]])
            yaw_array = np.array([[np.cos(yaw), -np.sin(yaw), 0], [np.sin(yaw), np.cos(yaw), 0], [0, 0, 1]])
        ...
        for pt in pc2.read_points(cloud_msg, field_names=("x", "y", "z"), skip_nans=True):
            ...
            rotated_point = rotate_point_euler(x, y, z, euler_angles[0], euler_angles[1], euler_angles[2])
    ...
    def perform_icp(self, previous_points, current_points, max_iterations=20, tolerance=1e-4):
        ...
        def svd_estimation(previous_points, current_points):
            ...
            # Center both
            P = current_points - source_centroid
            Q = previous_points - target_centroid
